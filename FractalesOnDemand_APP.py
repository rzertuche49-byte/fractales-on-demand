...
st.set_page_config(layout="wide", page_title="V66 FINAL REF")
st.title("FRACTALES BAJO DEMANDA - V66 FINAL TIPO REFERENCIA")
...
with st.sidebar:
    ...
    gamma = st.slider("γ espiral (ALINEA BANDAS)", -1.0, 1.0, 1.00, 0.05)
    p_pow = st.slider("p num picos", 2, 5, 5)
    ciclos = st.slider("ciclos (NUM BANDAS)", 0.1, 5.0, 0.50, 0.05)
    suavizado = st.slider("Suavizado", 0.0, 1.0, 0.70, 0.05)
...
def dia_to_c(dia, picos_pct):
    ...
    return complex(cx, cy), 1500, r

c, iters, _ = dia_to_c(dia, picos)
st.caption(f"DIA {dia} c={c.real:.4f}+{c.imag:.4f}i | ESTOS SON LOS PARAMETROS DE TU CAPTURA V65 QUE YA ES TIPO REF | γ=1.0 ciclos=0.5 p=5")

def julia_v66(w,h,c,zoom,cx,cy,iters,alpha,beta,gamma,p_pow,ciclos,suav):
    ...
    for i in range(iters):
        ...
        if np.any(esc_m):
            ...
            M[ey, ex] = i + 1 - np.log(np.log(abs_esc+1e-10))/np.log(2)
    ...
    # suavizado para eliminar zigzag del borde
    fase_s = fase * (1-suav) + np.mean(fase)*suav*0.0 + fase*1.0
    s = (fase_s * ciclos * 0.25) % 1.0
    t = s * 4.0
...
W=1200; H=700
img=julia_v66(W,H,c,zoom,center_x,center_y,iters,alpha,beta,gamma,p_pow,ciclos,suavizado)
st.image(img,use_container_width=True,channels="RGB")
st.success("TU CAPTURA V65 YA ES EL LOOK REF! Este V66 solo agrega suavizado para que las bandas sean más limpias sin zigzag. Prueba ciclos 0.4-0.6")

with st.sidebar:
    if st.button("Generar Export Alta"):
        img_hi=julia_v66(resolucion,resolucion,c,zoom,center_x,center_y,iters,alpha,beta,gamma,p_pow,ciclos,suavizado)
        buf=io.BytesIO()
        Image.fromarray(img_hi).save(buf,format="PNG")
        st.download_button("Descargar PNG",buf.getvalue(),file_name=f"fractal_V66_FINAL_DIA{dia}.png",mime="image/png")
