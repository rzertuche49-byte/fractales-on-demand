...
st.set_page_config(layout="wide", page_title="Fractales On Demand V100.6 MITAD VISIBLE")
...
def get_font_mono_fixed(size):
    ...
    try:
        from matplotlib import font_manager
        fp = font_manager.findfont("DejaVu Sans Mono", fallback_to_default=True)
        return ImageFont.truetype(fp, int(size))
    except Exception as e:
        try: return ImageFont.truetype("DejaVuSansMono.ttf", int(size))
        except: return ImageFont.load_default()

def crear_imagen_con_etiqueta_abajo(img_base, texto1, texto2, escala=1.0):
    W,H = img_base.size
    label_h = int(H * 0.14) # 14% - un poco mas para que entre texto
    nueva_h = H + label_h
    nueva = Image.new("RGB", (W, nueva_h), (255,255,255))
    # asegurar que fractal se vea
    ...
    # MITAD DE ANTES PERO VISIBLE - 1000px= 18px/11px, 7680px=138px/84px
    font_size_1 = max(int(W * 0.018), 12)
    font_size_2 = max(int(W * 0.011), 9)
    font1 = get_font_bold_fixed(font_size_1)
    font2 = get_font_mono_fixed(font_size_2)
    pad_x = int(W * 0.02)
    pad_y1 = int(label_h * 0.15)
    pad_y2 = int(label_h * 0.08)
    draw.text((pad_x, H+pad_y1), texto1, fill=(0,0,0), font=font1)
    try:
        bbox1 = draw.textbbox((pad_x, H+pad_y1), texto1, font=font1)
        h1 = bbox1[3]-bbox1[1]
    except:
        h1 = font_size_1
    draw.text((pad_x, H+pad_y1+h1+pad_y2), texto2, fill=(20,20,20), font=font2)
    return nueva
...
if presentar_etiqueta:
    st.markdown(f"""
    <div style="background:white;padding:10px 18px 12px 18px;border-radius:0 0 12px 12px;border:1px solid #E8E8E8;border-top:none;margin-top:-4px;">
        <div style="color:black;font-weight:800;font-size:13px;letter-spacing:0.1px;line-height:1.2;">{texto1}</div>
        <div style="color:#111;font-family:monospace;font-size:10px;margin-top:4px;line-height:1.2;letter-spacing:0.2px;">{texto2}</div>
    </div>
