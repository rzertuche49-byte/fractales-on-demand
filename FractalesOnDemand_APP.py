cat > FractalesOnDemand_APP.py << 'PYEOF'
...
st.title("FRACTALES BAJO DEMANDA - V26 SILUETA GORDA + SEDA")
...
st.write(f"c = {c} | V26 - Fix confeti interior")
...
def julia_v26(w, h, c, zoom, iters, vetas, suavidad, rotacion, brillo, modo):
    ...
    if "Silueta" in modo:
        # FIX CONFETI: todo lo que tardo > 25 en escapar = NEGRO GORDO (como V24)
        interior_gordo = M > 25
        exterior_muy_lejano = M < 2
        img[interior_gordo] = [0,0,0]
        img[exterior_muy_lejano] = [0,0,0]
        # Fade solo en anillo 2-25 para que no haya salto duro
        fade = np.clip((M - 2) / 23.0, 0, 1)
        # Suavizar fade para que no se vea banda cafe
        fade = np.sqrt(fade)
        img_float = img.astype(float) * fade[:,:,None]
...
st.success("V26 = Silueta gorda M>25 = sin confeti + exterior seda Af+log_r")
...
if st.sidebar.button("Generar Export Alta"):
    with st.spinner(f"Generando {resolucion}x{resolucion}..."):
        ...
        st.sidebar.download_button("Descargar PNG", buf.getvalue(), file_name=f"fractal_V26_SILUETA_DIA{dia}.png", mime="image/png")
PYEOF

git add FractalesOnDemand_APP.py
git commit -m "V26 fix confeti interior M>25 negro gordo + exterior seda"
...
rm FractalesOnDemand_APP.py
nano FractalesOnDemand_APP.py
# pega el codigo de arriba, Ctrl+O, Enter, Ctrl+X
git add FractalesOnDemand_APP.py
git commit -m "V26 fix heredoc"
git push origin main
