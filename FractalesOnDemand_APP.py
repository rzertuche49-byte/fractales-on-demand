...
st.set_page_config(layout="wide", page_title="V82 Editor")
st.title("FRACTALES V82 - EDITOR DE COLORES")
...
with st.sidebar:
    ...
    st.write("---")
    st.write("**EDITA LOS 6 COLORES:**")

    # Colores base
    base = PALETAS[paleta_nombre]

    c1 = st.color_picker("Color 1", base[0], key="c1")
    c2 = st.color_picker("Color 2", base[1], key="c2")
    c3 = st.color_picker("Color 3", base[2], key="c3")
    c4 = st.color_picker("Color 4", base[3], key="c4")
    c5 = st.color_picker("Color 5", base[4], key="c5")
    c6 = st.color_picker("Color 6", base[5], key="c6")
...
# Mostrar códigos para copiar
st.code(f"Paleta actual = {colores_actuales}")

buf = io.BytesIO()
Image.fromarray(out).save(buf, format="PNG")
st.sidebar.download_button("📥 Descargar PNG", buf.getvalue(), "fractal_custom.png", "image/png", type="primary")
