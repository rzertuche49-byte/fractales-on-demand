...
st.set_page_config(layout="wide", page_title="FRACTALES ON DEMAND V7")
st.title("FRACTALES ON DEMAND - V7 VETAS")
...
with st.sidebar:
    ...
    st.divider()
    paleta_nombre = st.selectbox("Estilo de Color", ["PSICODELICO (como tu foto)", "ARCOIRIS PURO", "FUEGO NEON", "OCEANO ACIDO"])
    fondo_transparente = st.checkbox("Fondo Transparente", value=False)
    potencia_color = st.slider("Intensidad Color", 0.5, 8.0, 5.0)
    rotacion_color = st.slider("Rotacion de Tono", 0.0, 1.0, 0.33)
    vetas = st.slider("Vetas / Ondas (nuevo)", 0.0, 10.0, 5.0)
...
st.write(f"DIA {dia} | VETAS {vetas}")

def crear_colormap(nombre):
    if nombre == "PSICODELICO (como tu foto)":
        colors = ["#ff00cc", "#ffcc00", "#00ffea", "#7a1fa2", "#ff00cc", "#ffff00", "#ff0066", "#00ff88"]
        return LinearSegmentedColormap.from_list("psy", colors, N=2048)
    elif nombre == "ARCOIRIS PURO":
        return plt.cm.hsv
    elif nombre == "FUEGO NEON":
        colors = ["#ff0000", "#ff8800", "#ffff00", "#ffffff", "#ff00ff"]
        return LinearSegmentedColormap.from_list("fuego", colors, N=1024)
    else:
        return plt.cm.turbo

def julia_psy(w, h, c, zoom, iters, cmap, potencia, rotacion, ondas, transparente):
    x_range = 3.0 / zoom
    y_range = 3.0 / zoom
    ...
    pattern = np.sin(smooth * ondas * 0.2 + np.angle(Z)*3) * 0.5
    norm = (smooth / iters) * potencia + rotacion + pattern
    norm = norm % 1.0
...
img_preview, mask = julia_psy(W, H, c, zoom, iters, cmap, potencia_color, rotacion_color, vetas, fondo_transparente)
...
if st.sidebar.button("Generar Export Alta"):
    with st.spinner(f"Generando {resolucion}x{resolucion}..."):
        img_hi, mask_hi = julia_psy(resolucion, resolucion, c, zoom, iters, cmap, potencia_color, rotacion_color, vetas, fondo_transparente)
        ...
        st.sidebar.download_button("Descargar PNG", buf.getvalue(), file_name=f"fractal_DIA{dia}_V7.png", mime="image/png")
